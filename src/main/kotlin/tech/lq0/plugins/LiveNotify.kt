...
class LiveNotify @Autowired constructor(app: Application) {
    ...
    suspend fun OneBotNormalGroupMessageEvent.showConfig() {
    ...
    suspend fun OneBotNormalGroupMessageEvent.showSubscribeCommand() = showAllSubscribe()

    @Listener
    @RequireBotAdmin
    @ChinesePunctuationReplace
    @Filter("!showsubscribe {{group,\\d+}}")
    suspend fun OneBotMessageEvent.showAnySubscribeCommand(@FilterValue("group") group: String) {
        showAnySubscribe(group)
    }
    ...
    suspend fun OneBotNormalGroupMessageEvent.clearSubscribe() = clearAnySubscribe("", groupId.toString())
