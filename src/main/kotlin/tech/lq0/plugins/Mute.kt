...
class Mute {
    ...
    suspend fun OneBotNormalGroupMessageEvent.randomMute(selfMute: Boolean = false) {
        val time = timeRange.random()
        val target = getFirstAtOrAuthor(selfMute) ?: return
    ...
    private suspend fun OneBotNormalGroupMessageEvent.getFirstAtOrAuthor(selfMute: Boolean = false): OneBotMember? {
        if (selfMute) return author()
    ...
    @ContentTrim
    @Filter("禁言抽奖")
    suspend fun OneBotNormalGroupMessageEvent.randomMuteCommand() {
        randomMute()
    }
